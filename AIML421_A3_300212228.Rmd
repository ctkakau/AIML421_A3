...
We can examine pairs of features to check for correlation to identify highly correlated features that may present options for removal .  We run scatter plots across all features but highlight only three interesting insights (see Figure  \@ref(fig:scattersTempo)):  
  ...
  2. 'popularity' feature has some weaker predictive power but still exhibits some clustering of genres, when combined with some features   
  3. some feature pairs appear to have positive correlation (e.g. 'energy' & 'loudness', 'danceability' & 'valence'), some negatively featured (e.g. 'acousticness' & 'loudness', and 'acousticness' & 'energy')    
...
The CONV1D layer runs an 8-bit kernel across every 'pixel' or datapoint in the set and calculates an average output across those bits.  Those outputs are then fed to the next dense layer of 128 nodes and on to the classification output layer.  

This network achieved around 54% accuracy after around 5 epochs (with a dataset of 12,500 training instances).  Which wasn't very good.  
...
After spending lots of time making dumb, time-eating decisions, I just threw the data into sklearn, let the model to the encoding, imputing missing values, scaling data, handling feature selection, training a default neural network and then running the prediction from that model.  It takes several minutes but selects a model that predicts with 63% accuracy.
...
I have seen this dataset before so I know there are several features categorical features that are ordinal, so I would have to figure out a way to capture that ordinality (ratings that are sorted alphabetically by default) which will probably require a dedicated step in preprocessing.  There are also several missing values so I would use a SimpleImputer to impute those - because it is quick and fairly straightforward.  
